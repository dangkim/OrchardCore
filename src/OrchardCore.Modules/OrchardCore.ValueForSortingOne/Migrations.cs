...
namespace OrchardCore.ValueForSortingOne
{
    public class Migrations : DataMigration
    {
        ...
        public int Create()
        {
            _contentDefinitionManager.AlterPartDefinition("ValueForSortingOnePart", builder => builder
                .Attachable()
        ...
        public async Task<int> UpdateFrom1()
        {
            ...
            for(;;)
            {
                ...
                foreach(var contentItemVersion in contentItemVersions)
                {
                    if (String.IsNullOrEmpty(contentItemVersion.DisplayText)
                        && UpdateValueForSortingOne(contentItemVersion.Content))
                    {
                        _session.Save(contentItemVersion);
                        _logger.LogInformation($"A content item version's ValueForSorting was upgraded: '{contentItemVersion.ContentItemVersionId}'");
                    }
            ...
            bool UpdateValueForSortingOne(JToken content)
            {
                var changed = false;

                if (content.Type == JTokenType.Object)
                {
                    var valueForSortingOne = content["ValueForSortingOnePart"] ? ["ValueForSortingOne"]?.Value<string>();
                    
                    if (!String.IsNullOrWhiteSpace(valueForSortingOne))
                    {
                        content["ValueForSortingOne"] = valueForSortingOne;
                        changed = true;
                    }
                }
                ...
                {
                    changed = UpdateValueForSortingOne(token) || changed;
                }
